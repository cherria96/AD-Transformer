...
class Model(nn.Module): 
    ...
    def __init__(self, configs):
        ...
        self.final_projection = nn.Linear(configs.pred_len, configs.pred_len)
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
        ...
        enc_out = self.enc_embedding(x_enc, x_mark_enc) # (b, context, enc_in)
        enc_out, attns = self.encoder(enc_out, attn_mask=enc_self_mask)

        dec_out = self.dec_embedding(x_dec, x_mark_dec)
        dec_out = self.decoder(dec_out, enc_out, x_mask=dec_self_mask, cross_mask=dec_enc_mask)
        
        # dec_out = dec_out[:, -self.pred_len:, :].permute(0,2,1)
        # dec_out = self.final_projection(dec_out).permute(0,2,1)
